...
all) the row reports, across that criterion's repeats (paired by repeat index):
  ...
  * how often norm-on beats / equals / trails norm-off, and
...
def compare(files, att, ff, res, bias, act):
    ...
    (on_better, equal, off_better, mean_pct_diff), comparing norm on vs off
...
def generate(cols):
    ...
    n_fixed = 5    # Attn, FF, Res, Bias, Act
    ...
    header2 = [r"\textbf{Attn}", r"\textbf{FF}", r"\textbf{Res}", r"\textbf{Bias}", r"\textbf{Act}"]
    for _crit in crits:
