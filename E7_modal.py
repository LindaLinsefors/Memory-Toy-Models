...
model_type = "full"          # 'simple' or 'full' 


log_to_wandb = True
lr = [1e-2, 3e-3, 1e-3]
patience = 5000
n_epochs = 50000                # Max epoch for each lr in each attempt. Early stopping may stop earlier.
...
# Base architecture for this experiment.
    ...
    raise ValueError("Invalid model type. Choose 'full', 'simple'.")
...
def build_configs():
    ...
    for d in d_values:
        ...
        if not testing:
            n_epochs = 50000 if d == 16 else 50000 * 2 if d == 32 else 50000 * 4 if d == 64 else 50000 * 8
