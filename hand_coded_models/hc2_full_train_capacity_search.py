"""Capacity search for FullyTrainedModel2 (the WHOLE network trained), on Modal (GPU).

Mirror of hc2_random_up_capacity_search.py with the model swapped for
FullyTrainedModel2 (see hc2_full_train.py): same architecture and random
...
capacity_search_results_fulltrain.json.

Run it with:
    python -m modal run hand_coded_models/hc2_full_train_capacity_search.py
"""
...
GRIDS_DIR = os.path.join(RESULTS_DIR, "fulltrain_grids")


# ── Settings ──────────────────────────────────────────────────────────────────
n_attempts = 11

# Training recipe (see hc2_full_train.train_full_network).
N_EPOCHS = 5000
LR = 1e-2
PATIENCE = 100
...
def _init_seed(attempt):
    """Per-attempt seed for the up + down init — the only source of variation
    between attempts (the facts are fixed)."""
    return attempt


# ── Modal app + image ─────────────────────────────────────────────────────────
app = modal.App("hc2-full-train-capacity-search")

# Self-contained image: ships hc2_full_train.py + hc2_random_up.py (its base
# class) + the repo-root modules they import.
image = (
    ...
    .add_local_file(os.path.join(_HERE, "hc2_full_train.py"), "/root/hc2_full_train.py")
)
...
# ── Result caching (grid files: hc2_fulltrain_d{d}_nfacts{nf}.json) ────────────

def _grid_glob():
    return os.path.join(GRIDS_DIR, "hc2_fulltrain_d*_nfacts*.json")
...
def _save_records(d, n_facts, records, n_attempts):
    ...
    out_path = os.path.join(GRIDS_DIR, f"hc2_fulltrain_d{d}_nfacts{n_facts}.json")
...
CAPACITY_RESULTS_PATH = os.path.join(RESULTS_DIR, "capacity_search_results_fulltrain.json")
