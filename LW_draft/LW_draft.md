...
To test what parts of the model are important for the sequence memorization task, I made a transformer model, where every part of the model can be turned on or off. Then I trained all variants of this model and compared their performance.
...
- An MLP layer (on the last token position only since I'm not trying to predict intermediate tokens)
...
*Mixing* is the part of the model that combines the first token and second token information into the same residual stream. I have three different variants for this.
...
How does the number of learnable facts grow with model size? To find this out, I picked two of the very many model architecture varieties and scaled them up. 
...
[^prior_work]:
  ...
  * [Rotations in Superposition](https://www.lesswrong.com/posts/LZ7YMPJueB6qjL24n/rotations-in-superposition)
  ...
  [The above list should be in the footnote. If it doesn't work, I'll fix it on LW.]
...
I did also try assigning positive values everywhere else, but for the success criteria I use (looking at arg_max of the logits), adding these positive values makes no difference. I first noticed this empirically, but it's also a mathematical fact.
...
When testing the capacity of this model design, I always do a hyperparameter search over $S$ and $top\_fraction$. 
