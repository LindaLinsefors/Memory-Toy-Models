...
Inside the binary search, for each number of facts tested, I trained 11 models in parallel, with the exact same facts, but different random initialized weights. I used three different success criterions "Any", "Most" and "All", meaning that I said the model succeeded at learning all the facts, if it succeeded in any, most or all of the 11 trials.[^most]

[^most]: "Most" mean that there are more successes than failure, i.e. at least 6 out of 10.

***All tables (below) show all of "Any", "Most" and "All". However, the analasys in the text, and in the main post is based on the results for "Any" since that's the most stable setting.***

Each individual training run was done as follows. I used Adam with no weight decay, and with all facts included in every bach.[^bach] Each where initially trained with learning rate `lr=1e-2`, for up to 50.000 epochs, or until early stopping due to reaching full accuracy, or accuracy not improving for the last 5.000 epochs. If after this step, if the accuracy is less than 100% but above 95%, then training continues with `lr=3e-3`, with same stopping criterion. If still not at 100% accuracy, training continues with `lr=1e-3`, with same stopping criterion. 

[^bach]: I.e. baches and epochs are teh same thing.
...
![](table_attention.png)
*Table A.2: Dual embedding vs. uniform attention vs. full attention. The first set of columns shows the settings of the other architecture variables. 2E>U is the number of replications where 2Emb outperformed Unif Attn. The first %$\Delta$ is the average percentage more facts 2Emb does better than Unif Attn. $\Delta$ is the average number of facts 2Emb does better than Unif Attn. The next ther columns is the same, except instead comparing Unif Attn with Lrn Attn. Teh last three columns is the same expect instead comparing 2Emb with Lrn Attn.* 
...
### MLP
![](table_ff.png)
*Table A.3: MLP=✅ vs MLP=❌. The first set of columns shows the settings of the other architecture variables. 'On' is the number of replications where MLP=✅ outperformed MLP=❌. '=' is the number of replications both to equally well. 'Off' is the number of replications where MLP=❌ outperformed MLP=✅. Mean%$\Delta$ is the average percentage more facts MLP=✅ does better than MLP=❌. $\Delta$ is the average number of facts MLP=✅ does better than MLP=❌.* 
...
### Norms
![](table_norm.png)
*Table A.4: Norms=✅ vs Norms=❌. The first set of columns shows the settings of the other architecture variables. 'On' is the number of replications where Norms=✅ outperformed Norms=❌. '=' is the number of replications both to equally well. 'Off' is the number of replications where Norms=❌ outperformed Norms=✅. Mean%$\Delta$ is the average percentage more facts Norms=✅ does better than Norms=❌. $\Delta$ is the average number of facts Norms=✅ does better than Norms=❌.*
...
![](table_residual.png)
*Table A.5: Res=✅ vs Res=❌. The first set of columns shows the settings of the other architecture variables. 'On' is the number of replications where Res=✅ outperformed Res=❌. '=' is the number of replications both to equally well. 'Off' is the number of replications where Res=❌ outperformed Res=✅. Mean%$\Delta$ is the average percentage more facts Res=✅ does better than Res=❌. $\Delta$ is the average number of facts Res=✅ does better than Res=❌.*
...
### MLP Bias
![](table_bias.png)
*Table A.6: Bias=✅ vs Bias=❌. The first set of columns shows the settings of the other architecture variables. 'On' is the number of replications where Bias=✅ outperformed Bias=❌. '=' is the number of replications both to equally well. 'Off' is the number of replications where Bias=❌ outperformed Bias=✅. Mean%$\Delta$ is the average percentage more facts Bias=✅ does better than Bias=❌. $\Delta$ is the average number of facts Bias=✅ does better than Bias=❌.*
...
*Table A.6: GELU vs ReLU. The first set of columns shows the settings of the other architecture variables. 'G>R' is the number of replications where GELU outperformed ReLU. '=' is the number of replications both to equally well. 'R>G' is the number of replications where ReLU outperformed GELU. Mean%$\Delta$ is the average percentage more facts GELU does better than ReLU. $\Delta$ is the average number of facts GELU does better than ReLU.*
...
The data points in Figures 3 and 5, in the main post, is for the 'Any' condition. Below is the same two plots with 'Most' and 'All' included.
...
![](scaling.png)
*Figure B.2: Same as figure 5 in main post, but with "most" and "all" settings included. The fitted lines include all data points for each of "simple" or "full", aggregating over "any"/"most"/"all".*
