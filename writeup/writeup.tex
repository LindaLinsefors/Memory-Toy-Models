...
\begin{enumerate}
    \item \textbf{Embedding}: The input tokens are mapped into a $d_\text{residual}$-dimensional residual stream. The precise mechanism depends on whether attention is enabled. (See Section~\ref{sec:embedding} for details.)
    \item \textbf{Attention} (optional): If enabled, the embedded sequence is processed by a single self-attention layer. After the attention, only the last sequence position is retained. (See Section~\ref{sec:attention} for details.)
    \item \textbf{Layer Norm 1} (optional): An RMSNorm layer normalises the residual stream. This layer norm is present if and only if both attention and norms are enabled.
    \item \textbf{Feed-Forward block} (optional): If enabled, the activations are passed through either a GELU or ReLU block, with a residual connection around this block as an optional extra. (See Section~\ref{sec:ff} for details).
    \item \textbf{Layer Norm 2} (optional): An RMSNorm layer normalises the residual stream. This layer norm is present if and only if both feed-forward and norms are enabled.
...
\texttt{attention}           & True/False  & Enable causal self-attention \\
...
\texttt{ff\_activation\_type}& GELU/ReLU  & Activation function (GELU or ReLU) \\
\bottomrule
\end{tabular}
\caption{All possible settings for \textsc{MemoryToyModel}, and the different values used in experiments. The code allows the \emph{Data dimensions} and \emph{Internal dimensions} (including \texttt{seq\_len},\texttt{n\_heads} and \texttt{seed}) to be set to different values than those listed, but the ones listed are the only ones actually used. Feel free to experiment yourself with other values.}
\label{tab:settings}
\end{table}
...
Each input token is embedded via a learned token embedding $W_E \in \mathbb{R}^{V_\text{in} \times d_\text{residual}}$ and summed with a learned positional embedding $W_P \in \mathbb{R}^{T \times d_\text{residual}}$, where $T$ is the sequence length and $V_\text{in}$ is the input vocabulary size. The full embedded sequence is normalised with RMSNorm (if \texttt{norms = True}, otherwise the identity) and passed through a causal self-attention layer with $n_\text{heads}$ heads (Section~\ref{sec:attention}). Only the representation at the \emph{last} sequence position ($T$) is retained, combined with the embedding at the same position via a residual connection that is always active (not controlled by any flag):
\[
    x = e_T + \text{Attn}\!\left(\text{Norm}(e)\right)_T,
\]
where $e \in \mathbb{R}^{T \times d_\text{residual}}$ is the embedded sequence and subscript $T$ denotes the last position. Note that the attention operates on the \emph{full} sequence (all positions attend to their causal context), but only the output at position $T$ is used going forward.
...
The residual-stream vector is passed through a final RMSNorm (if \texttt{norms = True}) and then a linear projection $W_H \in \mathbb{R}^{V_\text{out} \times d_\text{residual}}$ (with bias when \texttt{bias = True}) to produce logits over the output vocabulary of size $V_\text{out}$. This final norm is always present regardless of whether attention or the feed-forward block is enabled.
...
\centering
\begin{tabular}{@{}ll l>{\raggedright\arraybackslash} m{9cm}@{}}
\toprule
\textbf{Parameter} & \shortstack[l]{\textbf{Supported} \\ \textbf{by code}} & \shortstack[l]{\textbf{Used in} \\ \textbf{experiments}} & \textbf{Description} \\
\midrule
\multicolumn{4}{@{}l}{\emph{Training parameters}} \\
\hdashline[1pt/3pt]
\texttt{n\_epochs}         & int            & 50\,000               & Maximum training epochs \\
\hdashline[1pt/3pt]
\texttt{lr}                & float          & 1e-2, 3e-3, 1e-3      & Learning rate \\
\hdashline[1pt/3pt]
\texttt{optimizer\_type}   & Adam, AdamW    & Adam                  & Optimiser \\
\hdashline[1pt/3pt]
\texttt{grad\_clip\_norm}  & float or None  & None                  & Gradient clipping norm \\
\hdashline[1pt/3pt]
\texttt{smoothing}         & float or None  & None                  & Label smoothing factor \\
\hdashline[1pt/3pt]
\texttt{early\_stopping}   & True/False     & True                  & Enable early stopping \\
\hdashline[1pt/3pt]
\texttt{patience}          & int            & 3\,000--5\,000        & Epochs without improvement before stopping \\
\hdashline[1pt/3pt]
\texttt{target\_accuracy}  & \parbox[c]{3cm}{\vspace{2pt}\raggedright\texttt{accuracy}, {\footnotesize\texttt{best\_guess\_accuracy}}\vspace{2pt}} & \texttt{accuracy} & Monitored metric for early stopping \\
\hdashline[1pt/3pt]
\texttt{loss\_type}          & BCE, CE        & BCE, CE                   & Loss function type (Binary Cross-Entropy or Cross-Entropy) \\
\midrule
\multicolumn{4}{@{}l}{\emph{Capacity search parameters}} \\
\hdashline[1pt/3pt]
\texttt{lr}                & list of floats          & [1e-2, 3e-3, 1e-3]            
& Learning rates, used sequentially. When progress has stalled or max epochs reached, continue with the next learning rate. \\
\hdashline[1pt/3pt]
\texttt{precision}                          & int            & 8            
& Binary search stops when the remaining search interval is smaller than this value. \\
\hdashline[1pt/3pt]
\texttt{number\_of\_attempts}               & int            & 1, 3         
& Number of times to try to learn each number of facts, re-starting with a new randomly initialised model each time. \\
\hdashline[1pt/3pt]
{\small\texttt{threshold\_to\_continue}}  & float          & 0.99, 0.95         
& If the monitored accuracy is below this threshold, when it's time to move on to the next learning rate, just give up instead. \\
\bottomrule
\end{tabular}
\caption{Training and capacity-search parameters. ``Supported by code'' lists the types or values accepted by \texttt{train\_model} and \texttt{find\_max\_facts}; ``Used in experiments'' lists the values actually used in the recorded runs.}
...
\toprule
\textbf{File} & \rothead{\texttt{patience}} & \rothead{\texttt{target\_accuracy}} & \rothead{\texttt{loss\_type}} & \rothead{\texttt{precision}} & \rothead{\texttt{number\_of\_attempts}} & \rothead{\texttt{threshold\_to\_continue}} \\
\midrule
\texttt{E1/experiment\_log.jsonl} & 5000 & ? & BCE & 8 & 1 & 0 \\
\texttt{E2/experiment\_log.jsonl} & 5000 & ? & BCE & 8 & 1 & 0.99 \\
\texttt{E2/experiment\_log\_3att.jsonl} & 3000 & ? & BCE & 8 & 3 & 0.99 \\
\texttt{E2/experiment\_log\_3att\_duplicate.jsonl} & 3000 & ? & BCE & 8 & 3 & 0.99 \\
\midrule
\texttt{E2/experiment\_log\_CE\_3att.jsonl} & 3000 & ? & CE & 8 & 3 & 0.95 \\
\texttt{E2/experiment\_log\_CE\_3att\_duplicate.jsonl} & 3000 & ? & CE & 8 & 3 & 0.95 \\
\midrule
\texttt{E3/full\_model\_log.jsonl} & 5000 & ? & BCE & [8, 32, 128, 512] & 3 & 0.99 \\
\texttt{E3/full\_model\_log\_duplicate.jsonl} & 5000 & ? & BCE & [8, 32, 128, 512] & 3 & 0.99 \\
\texttt{E3/reduced\_model\_log.jsonl} & 5000 & ? & BCE & [8, 32, 128, 512] & 3 & 0.99 \\
\texttt{E3/reduced\_model\_log\_duplicate.jsonl} & 5000 & ? & BCE & [8, 32, 128, 512] & 3 & 0.99 \\
\bottomrule
\end{tabular}
\caption{Per-file training and capacity-search settings for the experiment logs in \texttt{E1}, \texttt{E2}, and \texttt{E3}. Entries are left as placeholders for manual completion.}
\label{tab:file-training-params}
\end{adjustwidth}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Fixed settings for the experiment in Section~\ref{sec:e2-3att}. The architectural toggles (\texttt{attention}, \texttt{ff}, \texttt{bias}, \texttt{norms}, \texttt{ff\_residual}, and \texttt{ff\_activation\_type}) are varied across runs, as shown in Table~\ref{tab:e2-3att}.}
...
\midrule
\cmark & \cmark & \cmark & \cmark & \cmark & GELU & 568 & \textbf{576} & \fbox{248} & 528 & $\star$ \\
...
\xmark & \cmark & \xmark & \xmark & \cmark & ReLU & \textbf{552} & 544 & 544 & & $\star$ \\
...
\toprule
 & & & & & & \multicolumn{4}{c}{\textbf{Max Facts}} \\
\cmidrule(l){7-10}
\textbf{Attention} & \textbf{FF} & \textbf{Norms} & \textbf{Residual} & \textbf{Bias} & \textbf{Activation} & \textbf{3att\_L} & \textbf{Dup} & \textbf{3att} & \textbf{Orig} \\
\midrule
\cmark & \cmark & \cmark & \cmark & \cmark & GELU &  & 800 & 744 & \textbf{832} \\
\cmark & \cmark & \cmark & \cmark & \cmark & ReLU &  & \textbf{768} & \textbf{768} & 728 \\
\cmark & \cmark & \cmark & \cmark & \xmark & GELU & \textbf{832} & \textbf{832} & 648 & 600 \\
\cmark & \cmark & \cmark & \cmark & \xmark & ReLU & \textbf{744} & 736 & 736 & 520 \\
\hdashline[1pt/3pt]
\cmark & \cmark & \cmark & \xmark & \cmark & GELU &  & \textbf{816} & 800 & 600 \\
\cmark & \cmark & \cmark & \xmark & \cmark & ReLU &  & \textbf{720} & 712 & 448 \\
\cmark & \cmark & \cmark & \xmark & \xmark & GELU & \textbf{768} & 696 & 632 & 576 \\
\cmark & \cmark & \cmark & \xmark & \xmark & ReLU & 360 & 304 & \textbf{408} & 320 \\
\hdashline[1pt/3pt]
\cmark & \cmark & \xmark & \cmark & \cmark & GELU & \textbf{584} & 544 & 432 & \fbox{264} \\
\cmark & \cmark & \xmark & \cmark & \cmark & ReLU & \fbox{240} & 416 & \textbf{432} & \fbox{120} \\
\cmark & \cmark & \xmark & \cmark & \xmark & GELU & \fbox{344} & \textbf{592} & 544 & \fbox{248} \\
\cmark & \cmark & \xmark & \cmark & \xmark & ReLU & \textbf{584} & 472 & \fbox{232} & \fbox{224} \\
\hdashline[1pt/3pt]
\cmark & \cmark & \xmark & \xmark & \cmark & GELU & 464 & 408 & 336 & \textbf{512} \\
\cmark & \cmark & \xmark & \xmark & \cmark & ReLU & 312 & \fbox{248} & \fbox{\textbf{424}} & 320 \\
\cmark & \cmark & \xmark & \xmark & \xmark & GELU & \textbf{472} & 312 & 392 & \fbox{176} \\
\cmark & \cmark & \xmark & \xmark & \xmark & ReLU & 368 & \textbf{432} & 392 & 368 \\
\midrule
\cmark & \xmark & \cmark & N/A & N/A & N/A &  & 144 & 152 & \textbf{168} \\
\cmark & \xmark & \xmark & N/A & N/A & N/A &  & 112 & \textbf{144} & 112 \\
\midrule
\xmark & \cmark & \cmark & \cmark & \cmark & GELU & \textbf{1016} & \textbf{1016} & \textbf{1016} & \textbf{1016} \\
\xmark & \cmark & \cmark & \cmark & \cmark & ReLU & \textbf{1016} & \textbf{1016} & \textbf{1016} & \textbf{1016} \\
\xmark & \cmark & \cmark & \cmark & \xmark & GELU & \textbf{1016} & \textbf{1016} & \textbf{1016} & 1000 \\
\xmark & \cmark & \cmark & \cmark & \xmark & ReLU & \textbf{1016} & \textbf{1016} & \textbf{1016} & 960 \\
\hdashline[1pt/3pt]
\xmark & \cmark & \cmark & \xmark & \cmark & GELU & \textbf{1016} & \textbf{1016} & \textbf{1016} & \textbf{1016} \\
\xmark & \cmark & \cmark & \xmark & \cmark & ReLU & \textbf{1008} & 1000 & 1000 & \fbox{504} \\
\xmark & \cmark & \cmark & \xmark & \xmark & GELU & \textbf{896} & 864 & 840 & 864 \\
\xmark & \cmark & \cmark & \xmark & \xmark & ReLU & 408 & 464 & \textbf{544} & \fbox{272} \\
\hdashline[1pt/3pt]
\xmark & \cmark & \xmark & \cmark & \cmark & GELU & 960 & \textbf{1000} & 960 & 960 \\
\xmark & \cmark & \xmark & \cmark & \cmark & ReLU & \textbf{960} & \textbf{960} & 936 & \textbf{960} \\
\xmark & \cmark & \xmark & \cmark & \xmark & GELU & \textbf{1016} & 960 & 960 & 992 \\
\xmark & \cmark & \xmark & \cmark & \xmark & ReLU & \textbf{960} & 936 & 944 & 944 \\
\hdashline[1pt/3pt]
\xmark & \cmark & \xmark & \xmark & \cmark & GELU & \textbf{864} & 800 & 808 & 832 \\
\xmark & \cmark & \xmark & \xmark & \cmark & ReLU & 800 & 744 & \textbf{808} & 624 \\
\xmark & \cmark & \xmark & \xmark & \xmark & GELU & \textbf{744} & \textbf{744} & 688 & 680 \\
\xmark & \cmark & \xmark & \xmark & \xmark & ReLU & \textbf{648} & 600 & \textbf{648} & 400 \\
\midrule
\xmark & \xmark & \cmark & N/A & N/A & N/A &  & \textbf{552} & 520 & 544 \\
\xmark & \xmark & \xmark & N/A & N/A & N/A &  & \textbf{216} & \textbf{216} & \textbf{216} \\
\bottomrule
\end{tabular}
\caption{Maximum facts memorised per architectural configuration using Cross-Entropy loss ($d_{\text{residual}}=16$, $d_{\text{ff}}=16$). Columns CE\_Long, CE\_Dup, CE\_3att, and CE\_Orig correspond to \texttt{experiment\_log\_CE\_3att\_long\_duplicate}, \texttt{experiment\_log\_CE\_3att\_duplicate}, \texttt{experiment\_log\_CE\_3att}, and \texttt{experiment\_log\_CE}, respectively. Empty cells indicate configurations not covered by that run. The highest value in each row is shown in \textbf{bold} and outliers are highlighted with a \fbox{box}.}
\label{tab:e2-ce}
\end{table}
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Data and model dimension parameters} & \textbf{Small} &  \textbf{Medium} &  \textbf{Large} & \textbf{XL} \\
\midrule
\texttt{input\_vocab\_size}  & 32 & 64 & 128 & 256 \\
\texttt{output\_vocab\_size} & 16 & 32 & 64 & 128 \\
\texttt{d\_residual}         & 16 & 32 & 64 & 128 \\
\texttt{d\_ff}               & 16 & 32 & 64 & 128 \\
\bottomrule
\end{tabular}
\caption{Data and model dimensions across different experiment scales.}
\end{table}
...
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Parameter} & \textbf{Full Model} & \textbf{Reduced Model} \\
\midrule
\multicolumn{3}{@{}l}{\emph{Architectural toggles}} \\
\texttt{attention}           & \cmark & \xmark  \\
\texttt{ff}                  & \cmark & \cmark \\
\texttt{bias}                & \cmark & \cmark   \\
\texttt{norms}               & \cmark & \xmark   \\
\texttt{ff\_residual}        & \cmark & \xmark   \\
\texttt{ff\_activation\_type}& GELU & ReLU   \\
\bottomrule
\end{tabular}
\caption{The two model architectures used in the scaling experiment.}
\end{table}
